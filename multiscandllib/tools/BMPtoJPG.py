...
def bmp_to_jpg(data_dir):
    """Create a new folder including all images in JPG format"""
    ...
    if not os.path.exists(data_dir):
        ...
        print(parent)
        new_folder = os.path.join(parent, "new_"+foldername+"_jpg")
        print(new_folder)
    ...
    for dirname, dirnames, filenames in os.walk(data_dir):
        ...
        for filename in filenames:
            if ('.bmp' or '.BMP') in filename:
                ...
                new_img_path = os.path.join(new_folder, os.path.basename(dirname), os.path.splitext(filename)[0]+".jpg")
                new_img = img.save(new_img_path)
                print("JPG image created: ", new_img_path)
